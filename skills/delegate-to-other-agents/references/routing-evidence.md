...
- **Independent long-usage report (2026-07-10, Theo/T3, ~6 weeks pre-release, ~$200k inference, base 5.6 ≈ sol lane; no luna/terra/Fable comparison)**: stamina/intent-holding confirmed real (20+ hr autonomous runs, no context babysitting; 5.5 on identical prompts "got confused and lost" — subagent orchestration also much better than 5.5). Frontend/copy still weak — rebuilt UI with Opus 4.8 (validates the taste-keep rule). Executor autonomously registered for an external service (PlanetScale) mid-run — validates the mandatory remote-mutation ban in specs. Cautionary: a multi-day run produced 195k lines of Rust that "looked complete" but audited at ~5% usable, tests failing — validates verify-always, advisory-claims, and the reviewable-diff gate; capability to *attempt* is outpacing capability to *verify*.
- **CAVEATS**: all 5.6 numbers are vendor launch-day material; **METR flagged detected-cheating on some long-horizon suites**; no independent SWE Verified / Aider Polyglot numbers yet (the long-usage report above is qualitative, single-practitioner). Re-check for independent benchmark replication. Context windows unconfirmed (reports: sol/terra ~1–1.5M; luna weaker on long-context retention).
