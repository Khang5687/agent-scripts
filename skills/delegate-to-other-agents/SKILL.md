...
- **Pick** (from `list --json`): exclude flagged/expired/masked → prefer recently-observed accounts with headroom → stale observations count less → an unknown account gets one low-risk dispatch to learn its state. Never probe accounts speculatively; **quota errors are authoritative** — on one, `mark` the account (`5h` or `weekly` per the error; weekly-capped stays excluded until the weekly reset, a 5h reset won't help) and `mark <name> no-sol` when sol isn't rolled out there (capability, not quota).
...
- **Safety**: never switch the global auth while any codex process is running — switch between dispatches only; a long run crossing a reset is left alone, reroute the next attempt. Advanced (parallel lanes × accounts): give each lane its own `CODEX_HOME` with that account's auth — true parallel multi-account; sync tokens back per lane home after; worth it only for ≥2 independent lanes and ≥2 fresh accounts.
