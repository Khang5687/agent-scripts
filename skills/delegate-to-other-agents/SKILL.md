...
**Explicit-invocation override**: if the user invoked this skill by name or said "delegate this", they have decided to delegate — never refuse on cost/economics grounds ("too expensive", "faster to do it myself", tiny-edit overhead, the metered-vs-flat-rate framing). Cost informs *which* model/executor to pick (§1, §3), never *whether* to obey an explicit delegate. Only the safety keeps in step 1 still apply (session tools/secrets, remote mutations) plus the plan-must-be-freezable rule (§2.2); if none block it, delegate. Absent an explicit invocation, run the full keep/delegate judgment below.
